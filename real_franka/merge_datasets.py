...
def get_sorted_parquets(dataset_path: str):
    ...
    files = glob.glob(
        os.path.join(dataset_path, "data", "**", "episode_*.parquet"), recursive=True
    )
    files.sort(
...
def patch_stats_record(
    rec: dict, new_ep_idx: int, new_frame_offset: int, T: int, task_remap: dict
) -> dict:
    """
    ...
    if "index" in s:
        ...
        idx_std = float(
            np.std(np.arange(new_frame_offset, new_frame_offset + T, dtype=np.float64))
        )
        s["index"] = {
            "min": [idx_min],
            "max": [idx_max],
...
def src_video_path(
    dataset_path: str, chunk_size: int, ep_idx: int, video_key: str
) -> str:
...
def dst_video_path(
    output_path: str, chunk_size: int, ep_idx: int, video_key: str
) -> str:
    chunk_id = ep_idx // chunk_size
    return os.path.join(
        output_path,
...
def merge_datasets(dataset_paths: list, output_path: str, chunk_size: int = 1000):
    ...
    with open(os.path.join(out_meta_dir, "tasks.jsonl"), "w", encoding="utf-8") as f:
        for i, task in enumerate(all_task_strings):
            f.write(
                json.dumps({"task_index": i, "task": task}, ensure_ascii=False) + "\n"
            )
    ...
    for ds_idx, (ds_path, (info, tasks, episodes)) in enumerate(
        zip(dataset_paths, ds_metas)
    ):
        ...
        if not ep_stats:
            has_stats = False
            print(
                f"  [WARN] No episodes_stats.jsonl found in {ds_path}, stats will be skipped."
            )

        print(f"Dataset {ds_idx + 1}/{len(dataset_paths)}: {os.path.basename(ds_path)}")
        print(
            f"  {len(parquet_files)} episodes, episode offset = {global_ep_offset}, frame offset = {global_frame_offset}"
        )

        for local_idx, parquet_path in enumerate(
            tqdm(parquet_files, desc=f"  Merging [{ds_idx}]")
        ):
            orig_ep_idx = int(
                os.path.splitext(os.path.basename(parquet_path))[0].split("_")[-1]
            )
            ...
            new_index_col = pa.array(
                range(global_frame_offset, global_frame_offset + T), type=pa.int64()
            )
            old_task_indices = table.column("task_index").to_pylist()
            new_task_col = pa.array(
                [task_remap[ti] for ti in old_task_indices], type=pa.int64()
            )
            ...
            table = table.set_column(
                table.schema.get_field_index("index"), "index", new_index_col
            )
            table = table.set_column(
                table.schema.get_field_index("task_index"), "task_index", new_task_col
            )
            ...
            # --- Write parquet ---
            out_chunk_dir = os.path.join(
                output_path, "data", f"chunk-{new_chunk_id:03d}"
            )
            os.makedirs(out_chunk_dir, exist_ok=True)
            pq.write_table(
                table, os.path.join(out_chunk_dir, f"episode_{new_ep_idx:06d}.parquet")
            )
            ...
            with open(episodes_out_path, "a", encoding="utf-8") as f:
                f.write(
                    json.dumps(
                        {"episode_index": new_ep_idx, "tasks": ep_tasks, "length": T},
                        ensure_ascii=False,
                    )
                    + "\n"
                )
